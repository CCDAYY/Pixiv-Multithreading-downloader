...
def getpageids(sname: str, total: int, page: int | None = None, mode: str | None = None):
    ...
    id = search(name=sname, search_mode=mode, offset=page * 53)
    return id
...
def master_request(method: str, url: str, is_token: bool | True, header: dict | None = None,
                   parameters: dict | str | None = None, data: dict | None = None):
    acc_t = token()
    try:
        if method == "GET":
            if is_token:
                res = requests.get(url=url, params=parameters, headers=acc_t.get_access_token(), stream=True, timeout=3)
                if res.status_code in {200, 301, 302}:
                    return res
                else:
                    print("retry")
                ...
                res = requests.get(url=url, params=parameters, headers=header, stream=True, timeout=3)
                if res.status_code in {200, 301, 302}:
                    return res
                else:
                    print("retry")
            ...
            try:
                return requests.post(url=url, headers=header, params=parameters, data=data, timeout=3)
            except Exception as e:
                print("error： ", e)
                return requests.post(url=url, headers=header, params=parameters, data=data, timeout=3)
    except:
        print(traceback.format_exc(), "line master_request")
